...
################
# This training pipeline has an FNO-based ENCA architecture. From input time
# series to sufficient statistics space, then using the same noise vectors back
# to the reconstruction of the initial input signal.
...
class Architecture:
    ...
    This example uses 1D Fourier Neural Operator blocks for both the encoder and
    the explicit-noise conditional decoder.'''
    def __init__(
        self,
        ndims_latent,
        len_timeseries,
        ...
        self.fno_modes = min(fno_modes, self.len_timeseries // 2 + 1)
        if self.fno_modes < 1:
            raise ValueError("fno_modes must be at least 1.")
        self.fno_layers = fno_layers
        self.use_time_coordinate = use_time_coordinate

        self.encoder = self.encoder_fn()
        self.decoder = self.decoder_fn()

    def time_coordinate_layer(self, name):
        time_grid = tf.linspace(0.0, 1.0, self.len_timeseries)
        time_grid = tf.reshape(time_grid, [1, self.len_timeseries, 1])
    ...
    def encoder_fn(self):
        ...
        x = x_input
        if self.use_time_coordinate:
            t = self.time_coordinate_layer(name='encoder_time_coordinate')(x_input)
            x = tf.keras.layers.Concatenate(axis=-1, name='encoder_concat_time')([x, t])
        x = tf.keras.layers.Dense(self.fno_width, activation=None, name='encoder_lift')(x)
        for i in range(self.fno_layers):
            x = FNOBlock1D(self.fno_width, self.fno_modes, name=f'encoder_fno_block_{i+1}')(x)
        x = tf.keras.layers.GlobalAveragePooling1D(name='global_avg_pool')(x)
        x = tf.keras.layers.Dense(self.fno_width, activation='gelu', name='encoder_dense')(x)
    ...
    def decoder_fn(self):
        ...
        inputs = [tile_ldims_layer(latent_mappings), noise_vectors]
        if self.use_time_coordinate:
            t = self.time_coordinate_layer(name='decoder_time_coordinate')(noise_vectors)
            inputs.append(t)
        x = tf.keras.layers.Concatenate(axis=-1, name='concatenate_noise_latent_and_time')(inputs)
        x = tf.keras.layers.Dense(self.fno_width, activation=None, name='decoder_lift')(x)
        for i in range(self.fno_layers):
            x = FNOBlock1D(self.fno_width, self.fno_modes, name=f'decoder_fno_block_{i+1}')(x)
        x = tf.keras.layers.Dense(self.fno_width, activation='gelu', name='decoder_dense')(x)
...
class ExpSetup:
    def __init__(self):
        tag = "fno_smoke"   # change this when you test something new
...
def main():
    ...
    model_obj = Architecture(
        ...
        use_time_coordinate=args.use_time_coordinate,
    )
...
class Sampler:
    ...
    def build_model(self):
        self.model_obj = Architecture(
            ...
            use_time_coordinate=self.args.use_time_coordinate,
        )
