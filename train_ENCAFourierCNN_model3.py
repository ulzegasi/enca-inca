...
# Fourier-CNN ENCA variant added August 2026.
################
# This training pipeline uses an ENCA on log-magnitude Fourier spectra of
# solar-dynamo / SDDE time series. The convolutional encoder produces summary
# statistics and the noise-conditioned CNN decoder reconstructs the spectrum.
...
##################################################################################################
class Architecture:
    '''Convolutional Fourier encoder with a noise-conditioned CNN decoder.'''
    ...
    def encoder_fn(self):
        '''Input: [batch, num_fft_components, 1]; output: [batch, ndims_latent].'''
        x_input = tf.keras.layers.Input(
            shape=[self.num_fft_components, self.num_input_channels],
            name='fourier_log_amplitude',
        )
        x = tf.keras.layers.Conv1D(16, 3, padding='same', activation='relu', name='enc_conv_1')(x_input)
        x = tf.keras.layers.Conv1D(16, 3, padding='same', activation='relu', name='enc_conv_2')(x)
        x = tf.keras.layers.MaxPool1D(pool_size=2, name='enc_maxpool')(x)
        x = tf.keras.layers.Conv1D(32, 3, padding='same', activation='relu', name='enc_conv_3')(x)
        x = tf.keras.layers.Conv1D(32, 3, padding='same', activation='relu', name='enc_conv_4')(x)
        x = tf.keras.layers.Conv1D(
            self.ndims_latent, 3, padding='same', activation=None, name='latent_channels'
        )(x)
    ...
    def decoder_fn(self):
        '''Reconstruct a Fourier log-magnitude from summaries and simulator noise.'''
        ...
        summary_projection = tf.keras.layers.Dense(
            self.num_fft_components, activation=None, name='summary_projection'
        )(latent_mappings)
        summary_projection = tf.keras.layers.Reshape(
            [self.num_fft_components, 1], name='summary_projection_shape'
        )(summary_projection)

        def resize_noise(noise):
            # Match PyTorch interpolate(..., mode="linear") used by the reference model.
            noise_4d = tf.expand_dims(noise, axis=2)
            resized = tf.image.resize(
                noise_4d, [self.num_fft_components, 1], method='bilinear'
            )
            return tf.squeeze(resized, axis=2)

        noise_resized = tf.keras.layers.Lambda(resize_noise, name='resize_noise_to_spectrum')(noise_vectors)
        x = tf.keras.layers.Concatenate(axis=-1, name='concatenate_noise_and_summary')(
            [summary_projection, noise_resized]
        )
...
class ExpSetup:
    def __init__(self):
        ...
        max_rfft_components = self.len_timeseries // 2 + 1
        if self.num_fft_components > max_rfft_components:
            raise ValueError(
                f"num_fft_components ({self.num_fft_components}) exceeds the rFFT size "
                f"({max_rfft_components}) for len_timeseries={self.len_timeseries}."
            )

        self.batch_size = 300
        self.max_training_steps = int(1.2e6)
        self.freq_log = 500
        ...
        # Broad priors shared with train_MLP_model3.py for model comparison.
...
def main():
    ...
    class ChiSquareStatistic(tf.keras.losses.Loss):
        def call(self, y_true, y_pred):
            '''Implements a chi-square-like sum of squared relative errors.'''
    ...
    def loss_reconstruction_fn_legacy(x, x_pred, return_each_dim=False):
        if return_each_dim:
            ...
            for i in range(x.shape[-1]): # do over all channels of observation (it's just 1 in our example.)
                ...
                d['ChiSquare_x_ch_%d'%(i+1)] = ChiSquareStatistic(name='ChiSquare')(y_true=x[...,i], y_pred=x_pred[...,i]) / args.num_fft_components
            return d
        else:
            # return tf.keras.losses.MeanSquaredError(name='MSE')(x,x_pred)
            return ChiSquareStatistic(name='ChiSquare')(x,x_pred) / args.num_fft_components
...
class Sampler:
    ...
    def reconstruct(self, num_samples=10, params=None):
        '''Return reconstructed log-magnitude spectra with shape [num_samples, num_fft_components].'''
        if params is None: # if none, fallback to default generator of the class instance
            if self.iterator is None:
                raise AssertionError('iterator is not defined. You must provide a parameter dict for the generator.')
            iterator = self.iterator
    ...
    def decode(self, tuple_summary_and_noise):
        '''Reconstruct log-magnitude spectra from (latent_representations, noise_vectors).'''
