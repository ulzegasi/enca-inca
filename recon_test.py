#!/usr/bin/env python3
# Reconstruction test for ENCA: generate one or more series from chosen
# parameters, reconstruct them with the trained autoencoder, and plot
# mean trajectories with 10-90% bands.
...
def main():
    ...
    t = np.arange(len_timeseries) * saveat
    ...
    ax.fill_between(t, x_true_lo, x_true_hi, alpha=0.20, label="true 10-90%")
    ax.fill_between(t, x_pred_lo, x_pred_hi, alpha=0.20, label="reconstructed 10-90%")
    ax.plot(t, x_true_mean, label="true mean", linewidth=1.8)
    ax.plot(t, x_pred_mean, label="reconstructed mean", linewidth=1.8, alpha=0.95)
    ax.set_xlabel("time")
    ax.set_ylabel("signal")
    ax.legend(loc="best")
    ax.set_title(f"Reconstruction test @ step {step} ({ckpt_prefix})")
