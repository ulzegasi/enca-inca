#Author: Firat Ozdemir, May 2021, firat.ozdemir@datascience.ch
################
...
class ExpSetup:
    ...
    def __init__(self):
        self.logdir = '/tmp/model2_INCA'
        self.ndims_latent = 4 # #summary stats
...
def main(args):
    ...
    ###############################################
    gen_train = src.generators.DataGenerator_SolarDynamo_Simplified_BatchSampler(len_timeseries=args.len_timeseries, batch_size=args.batch_size, p0=p0, alpha1_lims=alpha1_lims, delta_lims=delta_lims, epsilon_lims=epsilon_lims)
...
class Sampler:
    ...
    def build_custom_generator(self, return_generator=False, **kwargs):
        ...
        generator = src.generators.DataGenerator_SolarDynamo_Simplified_BatchSampler(len_timeseries=self.args.len_timeseries, batch_size=self.mb_of_same_realizations, prng=prng, p0=p0, alpha1_lims=alpha1_lims, delta_lims=delta_lims, epsilon_lims=epsilon_lims, **kwargs) ## Although we can request mb_of_same_realizations custom, it's safer to fix it to initial setup, which will hopefully match the number used in training of the model.
