#!/usr/bin/env python3
# Reconstruction test for noise-conditioned Fourier-amplitude MLP: generate one or more
# observations from chosen parameters, convert them to the Hann-windowed log
# FFT amplitude representation, reconstruct that representation, and plot
# means with 10-90% bands.
...
def build_encoder_decoder(
    ...
    num_noise_fft_features = 2 * num_noise_fft_components * num_noise_channels
    ...
    noise_fft_features = tf.keras.layers.Input(
        shape=[num_noise_fft_features], name="noise_fft_real_imag"
    )
    y = tf.keras.layers.Concatenate(name="concatenate_latent_and_noise_fft")(
        [latent_mappings, noise_fft_features]
    )
    y = tf.keras.layers.Dense(128, activation="relu", name="dec_dense_1")(y)
    ...
    decoder = tf.keras.Model(inputs=(latent_mappings, noise_fft_features), outputs=y)
...
def noise_to_fft_features_np(
    noise: np.ndarray,
    num_noise_fft_components: int,
) -> np.ndarray:
    """Map raw [batch, time, 1] noise to orthonormal rFFT real/imaginary features."""
    ...
    spectrum = spectrum[:, :num_noise_fft_components]
...
def main():
    ...
    if representation_mode != "fourier_amplitude":
        raise ValueError(
            f"recon_test_mlp.py expects representation_mode='fourier_amplitude', got {representation_mode!r}. "
            "Use recon_test_enca.py for time-domain ENCA runs."
        )
    num_fft_components = int(hp.get("num_fft_components", 100))
    num_noise_fft_components = int(hp["num_noise_fft_components"])
    fft_log_eps = float(hp.get("fft_log_eps", 1e-8))
    len_timeseries = int(round(Tobs / saveat))
    observation_length = num_fft_components
    ...
    encoder, decoder = build_encoder_decoder(
        ...
        num_noise_fft_components=num_noise_fft_components,
    )
    ...
    for i in range(args.nseeds):
        ...
        noise_fft = noise_to_fft_features_np(
            noise,
            num_noise_fft_components=num_noise_fft_components,
        )
        x_pred = decoder((z_latent, noise_fft), training=False).numpy()[0]
    ...
    ax.legend(loc="best")
    ax.set_title(f"MLP-with-noise spectrum reconstruction test @ step {step} ({ckpt_prefix})")
