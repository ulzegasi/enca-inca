#!/usr/bin/env python3
# Reconstruction test for FNO ENCA: generate observations from chosen
# parameters and evaluate them in the reconstruction domain saved by training.
...
def normalized_mse(y_true, y_pred, eps=1e-3):
    scale = np.sqrt(np.mean(y_true**2, axis=1, keepdims=True))
    scale = np.maximum(scale, eps)
    return float(np.mean(((y_true - y_pred) / scale) ** 2))


def timeseries_to_fft_log_amplitudes_np(x, num_fft_components, fft_log_eps):
    """Map [batch, time] signals to Hann-windowed log FFT amplitudes."""
    x = np.asarray(x, dtype=np.float32)
    n_time = x.shape[1]
    window = np.hanning(n_time).astype(np.float32)
    amplitudes = np.abs(np.fft.fft(x * window[None, :], axis=1)) / float(n_time)
...
def main():
    ...
    if hp.get("representation_mode", "time") != "time":
        raise ValueError("recon_test_fno.py expects representation_mode='time'.")
    if hp.get("architecture_name") not in (None, "fno_enca"):
        raise ValueError(f"recon_test_fno.py expects architecture_name='fno_enca', got {hp.get('architecture_name')!r}.")
    ...
    len_timeseries = int(round(Tobs / saveat))
    reconstruction_loss_domain = hp.get("reconstruction_loss_domain", "time")
    valid_recon_domains = ("time", "fourier_log_amplitude")
    if reconstruction_loss_domain not in valid_recon_domains:
        raise ValueError(
            f"Unknown reconstruction_loss_domain={reconstruction_loss_domain!r}; "
            f"expected one of {valid_recon_domains}."
        )
    ...
    x_true_all = np.zeros((args.nseeds, len_timeseries), dtype=np.float32)
    x_pred_all = np.zeros((args.nseeds, len_timeseries), dtype=np.float32)
    params_true = None
    ...
    for i in range(args.nseeds):
        ...
        x_batch = x_true[np.newaxis, ...].astype(np.float32)
        ...
        x_true_all[i, :] = x_true[:, 0]
        x_pred_all[i, :] = x_pred

    if reconstruction_loss_domain == "fourier_log_amplitude":
        num_fft_components = int(hp.get("num_fft_components", 100))
        fft_log_eps = float(hp.get("fft_log_eps", 1e-8))
        y_true_all = timeseries_to_fft_log_amplitudes_np(
            x_true_all, num_fft_components, fft_log_eps
        )
        y_pred_all = timeseries_to_fft_log_amplitudes_np(
            x_pred_all, num_fft_components, fft_log_eps
        )
        ...
        baseline_name = "flat_mean_spectrum"
        title_domain = "spectrum"
    else:
        y_true_all = x_true_all
        y_pred_all = x_pred_all
        x_axis = np.arange(len_timeseries) * saveat
        x_label = "time"
        y_label = "signal"
        true_label = "true"
        pred_label = "reconstructed"
        baseline_name = "flat_mean_signal"
        title_domain = "time-domain"

    rmse_per_seed = np.sqrt(np.mean((y_pred_all - y_true_all) ** 2, axis=1))
    chi_per_seed = np.array([relative_chisq(y_true_all[i], y_pred_all[i]) for i in range(args.nseeds)])
    rmse = float(np.mean(rmse_per_seed))
    chi = float(np.mean(chi_per_seed))
    norm_mse = normalized_mse(
        y_true_all, y_pred_all, eps=float(hp.get("recon_scale_eps", 1e-3))
    )

    flat_mean_pred = float(np.mean(y_true_all))
    flat_mean_rmse_per_seed = np.sqrt(np.mean((y_true_all - flat_mean_pred) ** 2, axis=1))
    ...
    x_true_mean = np.mean(y_true_all, axis=0)
    x_true_lo = np.percentile(y_true_all, 10, axis=0)
    x_true_hi = np.percentile(y_true_all, 90, axis=0)
    x_pred_mean = np.mean(y_pred_all, axis=0)
    x_pred_lo = np.percentile(y_pred_all, 10, axis=0)
    x_pred_hi = np.percentile(y_pred_all, 90, axis=0)
    ...
    ax.legend(loc="best")
    ax.set_title(f"FNO {title_domain} reconstruction test @ step {step} ({ckpt_prefix})")

    summary = (
        f"tau={params_true[0]:.4g}, T={params_true[1]:.4g}, Nd={params_true[2]:.4g}, "
        f"sigma={params_true[3]:.4g}, Bmax={params_true[4]:.4g}\n"
        f"domain={reconstruction_loss_domain}, mean RMSE={rmse:.4g}, normMSE={norm_mse:.4g}\n"
        f"flat baseline RMSE={flat_mean_rmse:.4g}, performance={performance_vs_flat_mean:.4g}, "
        f"mean relative_chisq={chi:.4g}, "
    ...
    out_png = os.path.join(outdir, f"recon_fno_{ckpt_prefix}_step{step}_{param_tag}_{stamp}.png")
    ...
    print(
        ...
        f"mean_relative_chisq={chi:.6g}  {baseline_name}_baseline_RMSE={flat_mean_rmse:.6g}  "
        f"performance_vs_{baseline_name}_baseline={performance_vs_flat_mean:.6g}  "
