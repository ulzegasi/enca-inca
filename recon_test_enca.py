#!/usr/bin/env python3
# Reconstruction test for time-domain ENCA: generate one or more observations
# from chosen parameters, reconstruct them with the trained autoencoder, and
# plot means with 10-90% bands.
...
def main():
    ...
    if representation_mode != "time":
        raise ValueError(
            f"recon_test_enca.py expects representation_mode='time', got {representation_mode!r}. "
            "Use recon_test_mlp.py for Fourier-amplitude MLP runs."
        )
    len_timeseries = int(round(Tobs / saveat))
    observation_length = len_timeseries
    ...
    ax.legend(loc="best")
    ax.set_title(f"ENCA reconstruction test @ step {step} ({ckpt_prefix})")
    ...
    flat_mean_name = "flat_mean_signal"
    print(
